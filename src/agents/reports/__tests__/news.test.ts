...
import { NewsAgent } from '../news/index';
...
describe('NewsAgent', () => {
  ...
  beforeEach(() => {
    newsAgent = new NewsAgent('TestNewsAgent');
    vi.clearAllMocks();
  });

  it('should handle news request and generate a report', async () => {
    const mockData = { dateRange: '2023-01-01 to 2023-01-31' };
    const mockResponse = { text: 'News report content' };

    // Mock the generateText function to return a predefined response
    (generateText as Mock).mockResolvedValueOnce(mockResponse);

    await newsAgent.handleNewsRequest(mockData);
  ...
  it('should log the correct message when generating a report', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(`[TestNewsAgent] News report generated: ${mockResponse.text}`);
  });
});